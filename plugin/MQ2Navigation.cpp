...
static void CopyStringView(char* destination, size_t destinationSize, std::string_view source)
{
	if (!destination || destinationSize == 0)
		return;

	const size_t length = std::min(destinationSize - 1, source.size());
	if (length > 0)
...
static void CopyStringView(char (&destination)[Size], std::string_view source)
{
	CopyStringView(destination, Size, source);
}
...
void MQ2NavigationPlugin::Command_Navigate(std::string_view line)
{
	...
	CopyStringView(mutableLine, line);
...
void MQ2NavigationPlugin::AttemptClick()
{
	...
	if (profile->GetInventorySlot(InvSlot_Cursor))
		return;
...
EQSwitch* ParseDoorTarget(char* buffer, const char* szLine, int& argIndex)
{
	...
	for (int i = 0; i < pSwitchMgr->NumEntries; i++)
	{
		...
		if ((searchAny || !_strnicmp(theSwitch->Name, buffer, len))
			&& ((gZFilter >= 10000.0f) || ((theSwitch->Z <= pSpawn->Z + gZFilter)
				&& (theSwitch->Z >= pSpawn->Z - gZFilter))))
		{
			id = theSwitch->ID;
...
std::shared_ptr<DestinationInfo> MQ2NavigationPlugin::ParseDestinationInternal(std::string_view line, int& idx)
{
	...
	if (!_stricmp(buffer, "spawn"))
	{
		...
		CopyStringView(szSpawnSearch, tempView);
...
void MQ2NavigationPlugin::ParseOptions(std::string_view line, int idx, NavigationOptions& options, NavigationArguments* args)
{
	...
	if (pos != std::string_view::npos)
	{
		...
		CopyStringView(mutableLine, temp);
		idx = 1;
	}
